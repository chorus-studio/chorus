...
const cssPath = path.resolve('theming/theme.css')
const mapPath = path.resolve('theming/css-map.json')
const outputPath = path.resolve('src/entrypoints/theme.content.css')
...
async function main() {
    ...
    const transformed = css.replace(/\.(\w[\w-]*)/g, (match, className) => {
        ...
        return `${match} /* unconverted */` // mark unconverted classes with a comment
    })
