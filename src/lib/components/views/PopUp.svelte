...
<main
    class="relative flex {popup
        ? 'h-[160px] w-[300px]  bg-[var(--bg)] px-3.5 py-3'
        : 'h-fit w-full'} flex-col gap-1"
>
    ...
    <TimeProgress {port} pip={!popup} />
    <MediaControls {port} pip={!popup} />
    {#if !popup}
        <VolumeSlider {port} />
    {/if}
</main>
