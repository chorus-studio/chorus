<script lang="ts">
    ...
    const SVG_ICONS: Record<string, SvgElement[]> = {
        seek: [
            { d: 'M34.46,53.91A21.91,21.91,0,1,0,12.55,31.78' },
            { points: '4.65 22.33 12.52 32.62 22.81 24.75' }
        ],
        'block-track': [
            { d: 'M5.965 4.904l9.131 9.131a6.5 6.5 0 00-9.131-9.131zm8.07 10.192L4.904 5.965a6.5 6.5 0 009.131 9.131zM4.343 4.343a8 8 0 1111.314 11.314A8 8 0 014.343 4.343z', fillRule: 'evenodd', clipRule: 'evenodd' }
        ],
        play: [
...
>
    ...
    <svg
        ...
        class="size-[{size}px] {getFillColor(icon)} {isPlayPause
            ...
            : ''}"
    >
        {#each icon.startsWith('seek') ? SVG_ICONS.seek : SVG_ICONS[icon] as el}
            {#if 'points' in el}
                <polyline points={el.points} />
            {:else}
                <path d={el.d} fill-rule={el.fillRule} clip-rule={el.clipRule} />
            {/if}
        {/each}
    </svg>
</Button>
