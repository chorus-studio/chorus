...
<div
    class="group relative aspect-square size-32 rounded-md transition-transform hover:scale-105"
...
>
    ...
    {#if themeTypeIcon()}
        <div class="absolute right-1 top-1 z-20">
            <svelte:component this={themeTypeIcon()} class="size-3 opacity-60" style="color: {textColor}" />
        </div>
    {/if}

    <!-- Theme Name -->
    <span
        class="absolute bottom-1.5 left-0 right-0 z-[5] truncate px-1 text-center text-xs"
    ...
    <div
        ...
        style="background-color: {bgColor}"
    >
        <div style="background-color: {sidebarColor}" class="left-0 h-24 w-2.5 rounded-sm"></div>
        <div style="background-color: {sidebarColor}" class="left-1 top-2 h-24 w-24 rounded-sm"></div>
    </div>
