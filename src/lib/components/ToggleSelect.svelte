<script lang="ts">
    ...
    let { list, value, label = '', onValueChange, pip = false } = $props()
</script>

<div class="flex items-center justify-between gap-x-2">
    {#if label}
        <Label class="text-base font-semibold lowercase text-muted-foreground">{label}</Label>
    {/if}
