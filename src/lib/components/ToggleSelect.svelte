        ...
        {#each list as item (item.value)}
            <ToggleGroup.Item
                ...
                class={buttonVariants({
                    variant: 'secondary',
                    size: 'sm',
                    class: `size-7 min-w-7 gap-0 rounded-none ${item.value === value ? 'disabled:bg-green-600 disabled:opacity-90 data-[state=on]:bg-green-600 data-[state=on]:text-white data-[state=on]:hover:bg-green-700' : ''}`
                })}
            >
                {item.label}
            </ToggleGroup.Item>
