...
function calculateColourPercentage(
...
function kmeans(data: number[][], options: KMeansOptions) {
    const k = options.k
    ...
    for (let iter = 0; iter < maxIterations; iter++) {
        ...
        for (const point of data) {
            ...
            for (let i = 0; i < k; i++) {
                const dist = point.reduce(
                    (acc: number, dim: number, j: number) => acc + (dim - centroids[i][j]) ** 2,
                    0
                )
        ...
        const newCentroids = clusters.map((cluster) => {
            return cluster.reduce(
                (acc, point) =>
                    point.map((dim: number, i: number) => acc[i] + dim / cluster.length),
                Array(data[0].length).fill(0)
            )
        })

        const converged = centroids.every((centroid: number[], i: number) =>
            centroid.every(
                (dim: number, j: number) => Math.abs(dim - newCentroids[i][j]) < tolerance
            )
        )
...
function getPalette(pixels: Uint8ClampedArray): Colour[] {
    ...
    for (let i = 0; i < pixels.length; i += 4) {
        ...
        colourCounts[colour.toString()]
            ? colourCounts[colour.toString()]++
            : (colourCounts[colour.toString()] = 1)
    }

    const result = kmeans(pixelArray, { k: 100, tolerance: 100 })

    const palette = result.centroids.map((centroid: number[]) => {
        const colour = `rgb(${Math.round(centroid[0])}, ${Math.round(centroid[1])}, ${Math.round(
            centroid[2]
        )})`
        const percentage = calculateColourPercentage(pixels, centroid as RGB)
        return {
            colour,
            percentage,
            base: [Math.round(centroid[0]), Math.round(centroid[1]), Math.round(centroid[2])] as RGB
        }
    })
...
function calculateLuminance([r, g, b]: number[]) {
    ...
    return 0.2126 * rLinear ** gamma + 0.7152 * gLinear ** gamma + 0.0722 * bLinear ** gamma
}

function sortColoursByProximityAndLuminance(colours: Colour[]) {
    const colourGroups: Record<string, Colour[]> = {}
...
function getContrastColour(backgroundRGB: number[], textColourRGB: number[], targetContrast = 7) {
    function getRelativeLuminance(colourRGB: number[]) {
    ...
    function adjustChannelLight(currentValue: number, L2: number, L1: number) {
    ...
    function adjustChannelDark(currentValue: number, L2: number, L1: number) {
...
function getContrastRatio(foregroundRGB: number[], backgroundRGB: number[]) {
...
function calculateEuclideanDistance(rgb1: number[], rgb2: number[]) {
    return Math.sqrt((rgb2[0] - rgb1[0]) ** 2 + (rgb2[1] - rgb1[1]) ** 2 + (rgb2[2] - rgb1[2]) ** 2)
}

function findElementWithMaxDistance(initialRgb: number[], colours: { base: number[] }[]) {
    let maxDistance = -1
    let elementWithMaxDistance = null

    for (let i = 0; i < colours.length; i++) {
        const currentDistance = calculateEuclideanDistance(initialRgb, colours[i].base)
